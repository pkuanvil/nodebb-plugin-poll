...
Poll.getOption = async function (pollId, option, withVotes) {
	const results = await utils.promiseParallel({
		...
		voteCount: Poll.getOptionVoteCount(pollId, option),
	});
	if (results.votes) {
...
Poll.restore = async function (pollId) {
	return await Poll.setFields(pollId, ['edited', 'deleted'], [0, 0]);
};
...
Poll.setFields = async function (pollId, fields, values) {
	const data = {};
	for (let i = 0; i < fields.length; i++) {
		data[fields[i]] = values[i];
	}
	return await db.setObject(`poll:${pollId}`, data);
};
