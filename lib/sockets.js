...
Sockets.get = async function (socket, data) {
	...
	return await Sockets.get2(pollId, callerUid);
};

Sockets.get2 = async function (pollId, callerUid) {
	const guestShowVotes = await Config.settings.get('toggles.allowAnon');
	const pollData = await Poll.get(pollId, callerUid, !!callerUid || guestShowVotes);
	// Don't show vote uids for frontend
	pollData.options.forEach((option) => { option.votes = undefined; });
	// Hide voteCount for guest if !guestShowVotes
	if (!callerUid && !guestShowVotes) {
...
Sockets.getConfig = async function () {
	return await Config.settings.getObject();
};
...
Sockets.getOptionDetails = async function (socket, data) {
	if (!data || isNaN(parseInt(data.pollId, 10)) || isNaN(parseInt(data.optionId, 10))) {
		throw new Error('Invalid request');
	}

	// Show details only when logged in or guestShowVotes
	const guestShowVotes = await Config.settings.get('toggles.allowAnon');
	const result = await Poll.getOption(data.pollId, data.optionId, !socket.uid || guestShowVotes);
	// Don't show vote uids for frontend
...
Sockets.canCreate = async function (socket, data) {
	if (!data || (isNaN(parseInt(data.cid, 10)) && isNaN(parseInt(data.pid, 10)))) {
		throw new Error('Invalid request');
	}
